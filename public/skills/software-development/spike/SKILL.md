...
## Library/Tool evaluation references

When a spike involves evaluating a specific library or tool (install → test features → document quirks), save the findings as `references/<library>-<version>.md` under this skill. Include:

- Complete dependency installation chain
- API patterns that differ from documentation expectations
- Known pitfalls and anti-patterns
- Performance benchmarks observed

This keeps class-level knowledge accessible without creating a separate skill per library. See `references/scrapling-v0.4.8.md` for an example.
