...
12. **`comfy install` 可能留下不完整依賴** — 如果 `comfy launch` 報 `ModuleNotFoundError`
    （如 sqlalchemy），用 `comfy --skip-prompt install --nvidia --restore` 重新安裝依賴。
    `--restore` 會強制重新解析並下載所有 Python 套件，不影響已下載的模型。
