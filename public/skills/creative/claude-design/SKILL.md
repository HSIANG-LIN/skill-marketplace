...
**Screenshot-based iteration** (see `references/screenshot-iteration-loop.md`): for one-off artifacts where visual fidelity matters, run a screenshot QA loop — build, screenshot, review, fix, re-screenshot. Cap at 3 rounds. Disable for animated components.
...
- Screenshot iteration (see `references/screenshot-iteration-loop.md`): useful for visual fidelity but must be capped at 3 rounds and disabled for animated components. Temporary screenshots accumulate — clean up between major iterations.
- Don't use external image hotlinks for copyrighted content — they get blocked by CORS/403. Use gradient + emoji placeholders instead.
- Don't forget `event.stopPropagation()` on inner link buttons inside clickable cards.

## Dashboard Patterns

For dark-themed, card-based dashboards with filtering (entertainment, content, media), see `references/dashboard-patterns.md` for:
- CSS variable system for dark entertainment dashboards
- Month-based grouping with sticky filter bar
- Multi-filter stacking (month + platform + genre) with toggle behavior
- Reference link buttons on cards
- CORS-safe poster placeholder pattern
- Platform icon badges
- Card data structure template
